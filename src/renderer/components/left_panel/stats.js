    ...
    <div class="statsContainer"> 
        ...
        <div>
            ...
            <div v-for="stat in stat_object" class="stat">
                ...
                <div v-if="stats_display == 'EVs'"          class="stat_value stat_exp">{{game_properties.player.team[0]['statExp' + stat.name].value}}</div>
                ...
                <div v-if="stats_display == 'Vitamins'"     class="stat_value stat_multi_line percentage_evs2" style="font-size: 21px;"><span style="font-size: 31px;">{{usable_vitamins(game_properties.player.team[0][stat.statExpPath].value)}}</span><span style="font-size: 15px;">/10</span></div>
                <div v-if="stats_display == 'Vitamins'"     class="stat_value stat_multi_line ev_stats_gained2">{{game_properties.player.team[0]['statExp' + stat.name].value}}<span style="font-size: 13px;">/65535</span></div>
                
                <div v-if="stats_display == 'Detailed EVs'" class="stat_value stat_multi_line devs"           >{{game_properties.player.team[0]['statExp' + stat.name].value}}/65535</div>
                <div v-if="stats_display == 'Detailed EVs'" class="stat_value stat_multi_line percentage_evs" >{{decimalToPercentage(game_properties.player.team[0]['statExp' + stat.name].value / 65535) }}%</div>
                <div v-if="stats_display == 'Detailed EVs'" class="stat_value stat_multi_line ev_stats_gained">Stats: {{Math.floor(Math.sqrt(game_properties.player.team[0]['statExp' + stat.name].value) / 4)}}</div>
                
                <div v-if="stats_display == 'Badge Boosts'" class="stat_value stat_multi_line devs"           >Stat: {{game_properties.player.team[0][stat.path].value}}</div>
                <div v-if="stats_display == 'Badge Boosts'" class="stat_value stat_multi_line percentage_evs" >Boost: {{get_bb_stat(stat.name)}}</div>
                <div v-if="stats_display == 'Badge Boosts'" class="stat_value stat_multi_line ev_stats_gained">+{{(get_stat(stat.name) - game_properties.player.team[0][stat.path].value) / game_properties.player.team[0][stat.path].value*100}}%</div>
            </div>
        </div>
...
export default defineComponent({
    ...
    computed: {
        ...
        crit_rate() {
            return this.calculate_crit_rate(PokeData.getSpecies(this.meta.starter));
        },
        stats_header() {
        ...
        growthRate() {
            var species = this.meta.currentSpecies == 'Backport' 
                ? this.meta.starter 
                : this.meta.currentSpecies;
            // debugger
            return PokeData.getSpecies(species ?? this.meta.starter).growth_rate
        },
    },
    methods: {
        base_stats(statPath) {
        ...
        calculate_crit_rate(pkmnData) {
            ...
            if (baseSpeed) {
                ...
                return PokeData.getSpecies(this.meta.starter).crit_rate
            }
        },
        ...
        get_stat(stat_name) {
            if (this.meta.gameState == `Overworld` || this.meta.gameState == `To Battle`) {
                switch (stat_name) {
            ...
            if (this.meta.gameState == `Battle` || this.meta.gameState == `From Battle`) {
                switch (stat_name) {
        ...
        statLabelOpacity() {
            ...
            let faded_opacity = .1
            let mod_atk = this.game_properties.battle.yourPokemon.modStageAttack.value != '0'  && this.meta.gameState == 'Battle' 
                ? faded_opacity 
                : default_opacity;
            let mod_def = this.game_properties.battle.yourPokemon.modStageDefense.value != '0' && this.meta.gameState == 'Battle' 
                ? faded_opacity 
                : default_opacity;
            let mod_spc = this.game_properties.battle.yourPokemon.modStageSpecial.value != '0' && this.meta.gameState == 'Battle' 
                ? faded_opacity 
                : default_opacity;
            let mod_spe = this.game_properties.battle.yourPokemon.modStageSpeed.value != '0'   && this.meta.gameState == 'Battle' 
                ? faded_opacity 
                : default_opacity;
            let object = {
                "Hp":      default_opacity,
    ...
    mounted() {
        this.game_properties.overworld.map.change((new_property, old_property) => {
