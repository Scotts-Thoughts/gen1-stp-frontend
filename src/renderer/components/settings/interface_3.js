...
export default {
    ...
    data() {
        return {
            store: useUIStylesStore(),
        };
    },
    methods: {
        update(a, b) {
            this.store.update(a, b.target.value);
        },
        updateFloat(setting, event) {
            this.store.update(setting, parseFloat(event.target.value));
        },
        updateChecked(setting, event) {
            this.store.update(setting, event.target.checked);
        },
        updateInt(setting, event) {
            this.store.update(setting, parseInt(event.target.value));
        },
        async color_picker() {
            const color = await new EyeDropper().open().then(res => res.sRGBHex);
            this.store.update("overlay_color", color);
        },
    },
    computed: {
        style() {
