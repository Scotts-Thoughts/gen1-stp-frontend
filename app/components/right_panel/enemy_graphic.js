...
<div>
    ...
    <div class="enemyGraphic">
        <div v-for="[x, pokemon] in party" class="ePkmnStyle">
            <img v-if="mapper.properties.meta.gameName == 'Yellow'" class="ePkmnSprite" :style="pokemon.style.faint" :src="'images/pokemon/' + mapper.properties.battle.trainer.team[x]?.species + '.png'"/>
            <img v-else-if="mapper.properties.meta.gameName == 'Red and Blue'" class="ePkmnSprite" :style="pokemon.style.faint" :src="'images/pokemon/Red_and_Blue/' + mapper.properties.battle.trainer.team[x]?.species + '.png'"/>
            <div class="ePkmnSpecies opacityTransition" :style="pokemon.style.species">{{mapper.properties.battle.trainer.team[x]?.species}}</div>
            <div class="ePkmnLevel opacityTransition" :style="pokemon.style.species">Lv:{{mapper.properties.battle.trainer.team[x]?.level}}</div>
            ...
            <div class="ePkmnStatsStyle ePkmnHp   opacityTransition" :style="pokemon.style.text">{{mapper.properties.battle.trainer.team[x]?.maxHp}}</div>
            <div class="ePkmnStatsStyle ePkmnAtk  opacityTransition" :style="pokemon.style.text">{{activeSlot(x,batt.enemyPokemon.partyPos.value,"attack",mapper.properties.battle.trainer.team[x]?.attack,"enemyPokemon")}}</div>
            <div class="ePkmnStatsStyle ePkmnDef  opacityTransition" :style="pokemon.style.text">{{activeSlot(x,batt.enemyPokemon.partyPos.value,"defense",mapper.properties.battle.trainer.team[x]?.defense,"enemyPokemon")}}</div>
            <div class="ePkmnStatsStyle ePkmnSpc  opacityTransition" :style="pokemon.style.text">{{activeSlot(x,batt.enemyPokemon.partyPos.value,"speed",mapper.properties.battle.trainer.team[x]?.speed,"enemyPokemon")}}</div>
            <div class="ePkmnStatsStyle ePkmnSpe  opacityTransition" :style="pokemon.style.text">{{activeSlot(x,batt.enemyPokemon.partyPos.value,"special",mapper.properties.battle.trainer.team[x]?.special,"enemyPokemon")}}</div>
            <div class="ePkmnStatsStyle ePkmnCrit opacityTransition" :style="pokemon.style.text">{{enemy_crit_rate(mapper.properties.battle.trainer.team[x])}}%</div>
            ...
            <div>
                <transition name="fade">
                    <div v-show="speed_comparison_toggle == true" :style="pokemon.style.text" class="speed_comparison">{{speed_comparison(x, mapper.properties.battle.trainer.team[x]?.speed).comparison}}</div>
                </transition>
            </div>
        </div>
    </div> 
...
module.exports = {
    ...
    computed: {
        party() {
            const result = [];
            for(let i = 0; i < this.mapper.properties.battle.trainer.totalPokemon; i++) {
                result.push([
                    i, 
                    {
                        style: this.get_enemy_pkmn_styles(this.batt.trainer.team[i]),
                    }
                ]);
            }
            return result;
